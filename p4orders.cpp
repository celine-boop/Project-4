...
bool hasValidSyntax(const char orders[])
{
	...
	int k = 0;
	int orderslength = strlen(orders);
	while (k != orderslength)
	{
		...
		if (!isalpha(orders[k]))
			return false;
		k++;
		if (k == orderslength || !isalpha(orders[k]))
			return false;
		k++;
		...
		if (k == orderslength || !isdigit(orders[k]))
			return false;
		k++;

		// There might be more digits

		while (k != orderslength && isdigit(orders[k]))
			k++;

		// There must be a status code

		if (k == orderslength || (orders[k] != '+' && orders[k] != '-'))
			return false;
		k++;
	}
